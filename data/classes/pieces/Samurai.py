...
class Samurai(Piece):
	def __init__(self, pos, color, board):
		...
		img_path = 'data/imgs/' + color[0] + '_samurai.png'
	...
	def get_moves(self, board):
		...
		if self.color == 'white':
			if self.x + 1 < 8 and self.y - 1 >= 0 and self.x + 2 < 8 and self.y - 2 >= 0:
				...
				if square.occupying_piece != None:
					if square.occupying_piece.color != self.color:
						...
						if square.occupying_piece == None:
							output.append(square)
			if self.x - 1 < 8 and self.y - 1 >= 0 and self.x - 2 >= 0 and self.y - 2 >= 0:
				square = board.get_square_from_pos(
					(self.x - 1, self.y - 1)
				)
				if square.occupying_piece != None:
